...
def fall_decttion(pose_data, prev_pose_data, frame_shape, curr_time):
    print("Detecting falls")
    print(frame_shape)
    fall = False
    interval = 1
    x_vals = pose_data[::3] #landmark x - coordinates
    #print(f"X values: {x_vals}")
    x_vals = [(x / frame_shape[1]) for x in x_vals] #normalized to frames shape
    y_vals = pose_data[1::3] #landmark y - coordinates
    #print(f"Y values: {y_vals}")
    y_vals = [(y / frame_shape[0]) for y in y_vals] #normalized to frames shape
    #vis_vals = pose_data[2::3] #landmark visibility

    shoulder_right = (x_vals[2], y_vals[2])
    shoulder_left = (x_vals[5], y_vals[5])
    ...
    base = (centroid[0], min(y_vals)) # * frame_shape[1], * frame_shape[0]

    prev_distance = prev_pose_data['distance']
    prev_centroid = prev_pose_data['centroid']
    prev_time = prev_pose_data['time']
    distance = abs(math.dist(centroid, base))
    ...
    if (cap.isOpened() == False):   #check if videocapture not opened
        ...
        while(cap.isOpened): #loop until cap opened or video not complete
            ...
            if ret: #if success is true, means frame exist
                ...
                #fall detection
                if len(landmarks) == 51: # If we have old pose data
                    current_time = time.time()
                    print("Detecting fall")
                    fall, old_pose = fall_decttion(landmarks, old_pose, frame.shape, current_time)
